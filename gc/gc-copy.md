...
它的优点在于有利于 cache，且不需要每个对象一样大；

缺点在于，每次更新指针时要对表格区域执行大量的搜索查找，且都是 o(n) 级别的搜索（当然可以先排序，但优化下来也只是 lg 级别的，而且要对所有活动对象都执行 lg 级别的搜索）；另外是每次移动对象时，可能需要对表格也进行大量的移动操作；还有就是每个对象不能太小，因为对象被识别为垃圾后，可能要在该垃圾对象中存储定字长信息，所以太小的话是不适合的。
